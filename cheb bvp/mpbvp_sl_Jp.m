...
[x0_tau,i_tau,fi_tau,~,~] = po_delay_interp(U,T,M,tau);
...
for i = 1:length(tau)
    % Interpolation
    fi_k = squeeze(fi_tau(sl_ind*M,i,:)).'; % interpolation coefficients
    dfi_k = (D0.'*fi_k.').'; % time derivative of interpolation coefficients
    i_k = (i_tau(sl_ind*M,i)-1)*M*n+1:i_tau(sl_ind*M,i)*M*n; % indicies of interpolation segment elements
    dtau_k = -1/T(i_tau(sl_ind*M,i))*dtau(i,:);
    % Corresponding derivatives
    Jfd_in = Jf_mj(m_in,u_sl,ud_sl,i); % vector field Jacobian before event
    Jfd_out = Jf_mj(m_out,u_sl,ud_sl,i); % vector field Jacobian after event
