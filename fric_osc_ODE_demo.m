...
% keeping ODE formalism, there is no time delay in the system
...
% 3) sticking (x'=0)
% x(t) = 0
% Dimensionless (xu=F_0/k,tu=1/om_n), autonomous, 1st order form
% 1) positive sliding (x'>0)
% x1'(t) = x2(t)
% x2'(t) = -2*zeta*x2(t) - x1(t) + f_0*cos(x3(t)-om*tau) - 1
% x3'(t) = om
% 2) negative sliding (x'<0)
% x1'(t) = x2(t)
% x2'(t) = -2*zeta*x2(t) - x1(t) + f_0*cos(x3(t)-om*tau) + 1
...
%       h2(x(t),x(t-tau)) = -x1(t)-2*zeta*x2(t)+f_0*cos(x3(t)-om*tau) +- eta
% - phase reset 
%       h3(x(t),x(t-tau) = x3(t) 
...
% 4) eta = mu*m*g/F_0       dimensionless sliding friction force
% 5) eta = mu_0*m*g/F_0     dimensionless sticktion force


%% System definition
addpath('#demo');
sys.f = 'f_fricosc_ODE';        % vector field modes
sys.e = 'e_fricosc_ODE';        % event functions
sys.tau = 'tau_fricosc_ODE';    % time delays
...
y0 = @(t) [0.0 5.0 0.0];  % initial history function
...
figure(); plot_orb(orb1c); title('Corrected orbit');
% figure(); plot(err);
...
t_guess = [0.0, 0.1996, 2.2106, 4.3246, 4.5423, 8.7036]; % guess for event locations 
...
corr_opts.nr.maxiter = 20; % increase iteration step limit
...
figure(); plot_orb(orb_slc); title('Sliding orbit')
...
branch_sl1 = br12_cont(orb_slc,sys,[4 2],bifs);
