function [x1,err] = newton_iter(x0,f,Jf,opts)
...
for i = 1:opts.maxiter
    ...
    if opts.logs
        fprintf('   Iter %i, |dx|=%d, |F(x)|=%d, time %0.3f s\n',...
            i,norm(dx),norm(err),toc);
    end
    if opts.plots
        color = ones(1,3)*(opts.maxiter-i)/opts.maxiter;
        subplot(2,1,1)
        plot(abs(err),'.-','Color',color);
        title(sprintf('$||F(x_{%i})||$ = %d',i,norm(err)));
        subplot(2,1,2)
        plot(dx,'.-','Color',color);
        title(sprintf('$||\\Delta x_{%i}||$ = %d',i,norm(dx)));
        drawnow
        pause(1.0)
    end
    ...
    if i==opts.maxiter && norm(err)>1e3*opts.abstol
        warning('Maximum iteration number reached with error %d\n',norm(err)); 
    end
end
if opts.plots
    hold off;
